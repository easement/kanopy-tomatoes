# Kanopy Movie Scores Chrome Extension

A Chrome extension that automatically displays Rotten Tomatoes and Letterboxd scores on Kanopy movie pages.

## Features

- 🎬 **Auto-detection**: Automatically detects movie titles and years from Kanopy pages
- 🍅 **Rotten Tomatoes**: Shows Tomatometer (critics) and Audience scores
- 📽️ **Letterboxd**: Shows average rating and number of fans
...
3. **Dual Search**: Searches both Rotten Tomatoes and Letterboxd using the movie title
4. **Year Matching**: Uses the year to find the correct movie when multiple results exist
5. **Score Extraction**: Parses both sites to extract scores and ratings
6. **Display**: Shows both RT and Letterboxd scores in a beautiful, embedded container below the movie title
